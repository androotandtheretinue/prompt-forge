...
[Unreleased]
Added
Embedded `llms.txt` verbatim into the `<head>` of `index.html`, about 3 kB into the file. The application page is the URL people share and therefore the one an agent fetches, and it previously carried a pointer to the protocol rather than the protocol; one fetch of the page is now sufficient. It sits in an inert `<script type="text/plain">` rather than a hidden element — hidden rendered text is a cloaking pattern search engines penalise, and a visually-hidden div reachable by screen readers would read nine kilobytes of instructions aloud to someone who did not ask for them. The block is generated by `npm run vocabulary` and the audit fails if it drifts from `llms.txt` or is pushed deeper than 16 kB into the file.
