...
[Unreleased]
Changed
The page order now applies on both platforms. Subject, Signal Rig, strum hint, board — then everything optional. It was a phone-only rescue, which left two arrangements to keep in step and a desktop still scrolling past the chaos dial, the output format, the cards and the radar to reach a row you wanted to change. A wide screen made that read as a choice rather than a wall; it was the same fault, better hidden.
Renamed `data-mobile-order` to `data-section-order`, since it no longer describes a phone. What is left in the media query is density — the two-line category row, the button grids — rather than arrangement.
Folded the global controls into the Signal Rig and moved it to the top, under the subject. They were two panels with a page between them: the rig chose which axes were in play and the buttons acted on them, and you set one at the top while the other sat below the chaos dial. Scope and trigger now live in the same panel, in that order.
Removed UNLOCK ALL. `allCategoriesLive` and `unlockAll` were the same function written twice — the rig's copy simply forgot to release the subject — surfaced as ALL LIVE and UNLOCK ALL in two panels under two names. The difference between them was not a design decision, it was a dropped line. ALL LIVE is the survivor and it releases the subject too, which is what anyone pressing the other one wanted.
Fixed
Fixed the injected CLEAR UNLOCKED button being found by its layout classes. The interaction layer located the control row with `.flex.flex-wrap.gap-3.mb-4`, a selector made entirely of utilities, which named that row only for as long as nobody restyled it — and folding it into the rig changed exactly those classes. It would have stopped being injected with no error and nothing missing to look for. It is anchored by id now, and the audit refuses any element located by layout utilities.
Fixed the published vocabulary being short by 39 signals for four releases. PHOSPHOR's supporting vocabulary lived in the v5 interaction layer and was merged into the pools at runtime, where neither the generator nor the audit could see it — so the app offered 2,343 signals while vocabulary.json, vocabulary.txt, llms.txt, the README, the metadata and CITATION.cff all said 2,304. You could roll Phosphor Green off the palette axis and then be told by the published LEXICON that it was not a canonical value. The card and its vocabulary now live with the rest of the vocabulary; the interaction layer is for interaction.
PHOSPHOR was also the 29th Forge Card while every count said 28, for the same reason and since the same release.
The multiple-of-16 rule now applies to each axis bank rather than to the supplemented total, which is what it always meant — the convention governs the curated banks, and a themed supplement of one to five signals was never part of that arithmetic. The audit reports the bank size and the supplement separately so 178 never has to be read as a multiple of 16.
Added an audit check that every PHOSPHOR signal actually reaches the axis it names.
Corrected the medium bank in the README, which had said 160 since 5.2 and has been 176 since the motion mediums arrived.
Changed
The per-format subject override is now collapsed by default — most sessions never need it, and it sat above the board costing a third of the subject panel. It cannot hide while it is doing something: when an override is set the summary names it, the panel opens itself, and the audit checks that it does. A subject quietly substituted by a control folded out of sight is the same class of problem as the duplicate subject the override replaced.
