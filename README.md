...
- **Ctrl+Shift + any reroll** banks instead of surveying. A banked signal is filtered out of every draw whether or not a key is held, and stays out until you let it back in — the difference between "not this pass" and "not again". The **⚱ BANISHED SIGNALS** panel lists everything banked by axis, each entry its own undo, with one button to release the lot. The bank persists across sessions and refuses to take any axis below two signals, since nothing rescues a permanent ban.
